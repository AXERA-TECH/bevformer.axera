...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    def point_sampling(self, reference_points, pc_range, img_metas=None, 
                       lidar2img=None, img_shape=None):
        # NOTE: close tf32 here.
        allow_tf32 = torch.backends.cuda.matmul.allow_tf32
        torch.backends.cuda.matmul.allow_tf32 = False
        torch.backends.cudnn.allow_tf32 = False

        # Support both img_metas dict and direct tensor input
        if lidar2img is not None:
            if isinstance(lidar2img, torch.Tensor):
                lidar2img = lidar2img.to(reference_points.device).to(reference_points.dtype)
            else:
                lidar2img = reference_points.new_tensor(lidar2img)  # (B, N, 4, 4)
        else:
            # Fallback to img_metas
            if img_metas is not None:
                if torch.onnx.is_in_onnx_export():
                    lidar2img = img_metas[0]['lidar2img'] if isinstance(img_metas[0], dict) and 'lidar2img' in img_metas[0] else None
                else:
                    lidar2img = []
                ...
                raise ValueError("Either lidar2img or img_metas must be provided")
        ...
        bev_mask = bev_mask * (reference_points_cam[..., 1:2] > 0.0).float()
        bev_mask = bev_mask * (reference_points_cam[..., 1:2] < 1.0).float()
        bev_mask = bev_mask * (reference_points_cam[..., 0:1] < 1.0).float()
        bev_mask = bev_mask * (reference_points_cam[..., 0:1] > 0.0).float()
    ...
    def forward(self,
        ...
        # Extract lidar2img and img_shape from kwargs if available
        lidar2img = kwargs.get('lidar2img', None)
        img_shape = kwargs.get('img_shape', None)
        img_metas = kwargs.get('img_metas', None)
        
        reference_points_cam, bev_mask = self.point_sampling(
            ref_3d, self.pc_range, img_metas=img_metas, lidar2img=lidar2img, img_shape=img_shape)
