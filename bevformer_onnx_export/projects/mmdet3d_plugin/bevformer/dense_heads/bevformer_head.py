...
class BEVFormerHead(DETRHead):
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False, 
                lidar2img=None, can_bus=None, img_shape=None):
        """Forward function.
        Args:
            ...
            lidar2img (Tensor, optional): Lidar to image transformation matrices with shape (B, N, 4, 4).
            can_bus (Tensor, optional): CAN bus signals with shape (B, 18).
            img_shape (Tensor, optional): Image shape with shape (B, 2).
        Returns:
        ...
        if lidar2img is not None:
            kwargs['lidar2img'] = lidar2img
        if can_bus is not None:
            kwargs['can_bus'] = can_bus
        # img_shape is now fixed in encoder config, only pass if explicitly provided for backward compatibility
        ...
        if torch.onnx.is_in_onnx_export():
            ...
            for lvl in range(hs.shape[0]):
                ...
                outputs_references.append(reference)

            outputs_classes = torch.stack(outputs_classes)
            outputs_coords = torch.stack(outputs_coords)
            outputs_references = torch.stack(outputs_references)

            assert outputs_references.shape[-1] == 3
            tmp = outputs_coords
            tmp02 = tmp[..., 0:2]
            tmp02 = tmp02 + outputs_references[..., 0:2]
            tmp02 = tmp02.sigmoid()
            tmp01 = tmp02[..., 0:1]
            tmp01 = tmp01 * (self.pc_range[3] - self.pc_range[0]) + self.pc_range[0]
            tmp12 = tmp02[..., 1:2]
            tmp12 = tmp12 * (self.pc_range[4] - self.pc_range[1]) + self.pc_range[1]
            tmp45 = tmp[..., 4:5]
            tmp45 = tmp45 + outputs_references[..., 2:3]
            tmp45 = tmp45.sigmoid()
            tmp45 = tmp45 * (self.pc_range[5] - self.pc_range[2]) + self.pc_range[2]
            tmp = torch.cat([tmp01, tmp12, tmp[..., 2:4], tmp45, tmp[..., 5:]], dim=-1)
            outputs_coords = tmp
        else:
            for lvl in range(hs.shape[0]):
                if lvl == 0:
